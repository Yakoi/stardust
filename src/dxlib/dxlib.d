...
// ＤＸライブラリのバージョン
enum {
	DXLIB_VERSION = 0x306c,
}
const TCHAR[] DXLIB_VERSION_STR = "3.06c";
...
enum {
	...
	DX_HANDLETYPE_VERTEX_BUFFER =				(0x70000000),		// 頂点バッファハンドル
	DX_HANDLETYPE_INDEX_BUFFER =				(0x78000000),		// インデックスバッファハンドル
}
...
enum {
	...
	DX_GRAPH_FILTER_HSB =						(4),			// 色相・彩度・明度フィルタ
	DX_GRAPH_FILTER_INVERT =					(5),			// 階調の反転フィルタ
	DX_GRAPH_FILTER_LEVEL =						(6),			// レベル補正フィルタ
	DX_GRAPH_FILTER_TWO_COLOR =					(7),			// ２階調化フィルタ
	DX_GRAPH_FILTER_GRADIENT_MAP =				(8),			// グラデーションマップフィルタ
	DX_GRAPH_FILTER_NUM =						(9),
}
...
enum {
	...
	DX_GRAPHICSIMAGE_FORMAT_3D_DRAWVALID_ABGR_F16 =		(14),	// 描画可能ARGB浮動小数点16ビット型カラー
	DX_GRAPHICSIMAGE_FORMAT_3D_DRAWVALID_ABGR_F32 =		(15),	// 描画可能ARGB浮動小数点32ビット型カラー
	DX_GRAPHICSIMAGE_FORMAT_3D_NUM =					(16),
	DX_GRAPHICSIMAGE_FORMAT_2D =						(17),	// 標準( DirectDrawSurface の場合はこれのみ )
	DX_GRAPHICSIMAGE_FORMAT_R5G6B5 =					(18),	// R5G6B5( MEMIMG 用 )
	DX_GRAPHICSIMAGE_FORMAT_X8A8R5G6B5 =				(19),	// X8A8R5G6B5( MEMIMG 用 )
	DX_GRAPHICSIMAGE_FORMAT_X8R8G8B8 =					(20),	// X8R8G8B8( MEMIMG 用 )
	DX_GRAPHICSIMAGE_FORMAT_A8R8G8B8 =					(21),	// A8R8G8B8( MEMIMG 用 )

	DX_GRAPHICSIMAGE_FORMAT_NUM =						(22),	// グラフィックフォーマットの種類の数
}
...
enum {
	...
	DX_BASEIMAGE_FORMAT_ABGR_F16 =		(6),		// 16ビット不動小数点型
	DX_BASEIMAGE_FORMAT_ABGR_F32 =		(7),		// 32ビット不動小数点型
}
...
struct IMAGEFORMATDESC
{
	...
	ubyte					MSSamples ;						// マルチサンプリング数( 描画対象の場合使用 )
	ubyte					MSQuality ;						// マルチサンプリングクオリティ( 描画対象の場合使用 )
}
...
struct DINPUT_JOYSTATE
{
	...
	uint					POV[ 4 ] ;						// ハットスイッチ４つ( -1:入力なし 0:上 4500:右上 9000:右 13500:右下 18000:下 22500:左下 27000:左 31500:左上 )
	ubyte					Buttons[ 32 ] ;					// ボタン３２個( 押されたボタンは 128 になる )
}
...
extern	int			SetApplicationLogSaveDirectory( LPCTSTR DirectoryPath ) ;				// ログファイルを保存するディレクトリパスを設定する
...
extern	int			GetMouseWheelRotVol( int CounterReset = TRUE ) ;						// 垂直マウスホイールの回転量を得る
extern	int			GetMouseHWheelRotVol( int CounterReset = TRUE ) ;						// 水平マウスホイールの回転量を得る
...
extern	int			GetHostIPbyName( LPCTSTR HostName, IPDATA *IPDataBuf ) ;				// ＤＮＳサーバーを使ってホスト名からＩＰアドレスを取得する( IPv4版 )
extern	int			GetHostIPbyName_IPv6( LPCTSTR HostName, IPDATA_IPv6 *IPDataBuf ) ;		// ＤＮＳサーバーを使ってホスト名からＩＰアドレスを取得する( IPv6版 )
...
extern	int			GetJoypadDirectInputState( int InputType, DINPUT_JOYSTATE *DInputState ) ;	// DirectInput から得られるジョイパッドの生のデータを取得する( DX_INPUT_KEY や DX_INPUT_KEY_PAD1 を引数に渡すとエラー )
...
extern	int			SetFullSceneAntiAliasingMode( int Samples, int Quality ) ;																			// 画面のフルスクリーンアンチエイリアスモードの設定をする( DxLib_Init の前でのみ使用可能 )
...
extern	int			MakeScreen( int SizeX, int SizeY, int UseAlphaChannel = FALSE ) ;																							// 描画可能な画面を作成
...
extern	int			BltBmpToGraph( COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int CopyPointX, int CopyPointY, int GrHandle ) ;									// 画像データの転送
extern	int			BltBmpToDivGraph( COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int AllNum, int XNum, int YNum, int Width, int Height, int *GrHandle, int ReverseFlag ) ;	// 分割画像へのＢＭＰの転送
extern	int			BltBmpOrGraphImageToGraph( COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int BmpFlag,
										BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage,
										int CopyPointX, int CopyPointY, int GrHandle ) ;																						// ＢＭＰ か GraphImage を画像に転送
extern	int			BltBmpOrGraphImageToGraph2( COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int BmpFlag,
										BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage,
										RECT *SrcRect, int DestX, int DestY, int GrHandle ) ;																					// ＢＭＰ か GraphImage を画像に転送
extern	int			BltBmpOrGraphImageToDivGraph( COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int BmpFlag,
										BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage,
...
extern	int			DrawPrimitiveIndexed2D( VERTEX2D *Vertex, int VertexNum, ushort *Indices, int IndexNum, int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;				// ２Ｄプリミティブを描画する(インデックス)
...
extern	int			DrawPrimitiveIndexed3D( VERTEX3D *Vertex, int VertexNum, ushort *Indices, int IndexNum, int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;				// ３Ｄプリミティブを描画する(インデックス)
...
extern	int			DrawPolygonIndexed3DBase( VERTEX_3D *Vertex, int VertexNum, WORD *Indices, int IndexNum, int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;	// ３Ｄポリゴンを描画する(インデックス)( 旧バージョン用 )
...
extern	int			GetConstIndexToShader( LPCTSTR ConstantName, int ShaderHandle ) ;			// 指定の名前を持つ定数が使用するシェーダー定数の番号を取得する
extern	int			GetConstCountToShader( LPCTSTR ConstantName, int ShaderHandle ) ;			// 指定の名前を持つ定数が使用するシェーダー定数の数を取得する
extern	LPCFLOAT4   GetConstDefaultParamFToShader( LPCTSTR ConstantName, int ShaderHandle ) ;	// 指定の名前を持つ浮動小数点定数のデフォルトパラメータが格納されているメモリアドレスを取得する
...
extern	int			SetDrawAlphaTest( int TestMode, int TestParam ) ;					// 描画時のアルファテストの設定を行う( TestMode:DX_CMP_GREATER等( -1:デフォルト動作に戻す )  TestParam:描画アルファ値との比較に使用する値 )
...
extern	int			SetIgnoreDrawGraphColor( int EnableFlag ) ;							// 描画する画像の色成分を無視するかどうかを指定する( EnableFlag:この機能を使うかどうか( TRUE:使う  FALSE:使わない ) )
...
extern	int			SetGraphicsDeviceRestoreCallbackFunction( void function( void *Data ) Callback, void *CallbackData ) ;	// グラフィックスデバイスがロストから復帰した際に呼ばれるコールバック関数を設定する
...
extern	int			SetUseGraphZBuffer( int GrHandle, int UseFlag ) ;												// グラフィックにＺバッファを使用するかどうかを設定する( 描画可能画像のみ可能 )
extern	int			CopyGraphZBufferImage( int DestGrHandle, int SrcGrHandle ) ;									// グラフィックのＺバッファの状態を別のグラフィックのＺバッファにコピーする( DestGrHandle も SrcGrHandle もＺバッファを持っている描画可能画像で、サイズが同じであり、且つアンチエイリアス画像ではないことが条件 )

extern	int			GetGraphSize( int GrHandle, int *SizeXBuf, int *SizeYBuf ) ;									// グラフィックのサイズを得る
extern	int			GetGraphTextureSize( int GrHandle, int *SizeXBuf, int *SizeYBuf ) ;								// グラフィックに使用されているテクスチャーのサイズを得る
extern	int			GetGraphMipmapCount( int GrHandle ) ;															// グラフィックが持つミップマップレベルの数を取得する
...
extern	int			CreateGraphFromGraphImage( BASEIMAGE *Image, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																																					// GraphImageデータからグラフィックハンドルを作成する
extern	int			b2_CreateGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																															// GraphImageデータからグラフィックハンドルを作成する
extern	int			ReCreateGraphFromGraphImage( BASEIMAGE *Image, int GrHandle, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																																	// GraphImageデータから既存のグラフィックハンドルにデータを転送する
extern	int			b2_ReCreateGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int GrHandle, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																											// GraphImageデータから既存のグラフィックハンドルにデータを転送する
extern	int			CreateDivGraphFromGraphImage( BASEIMAGE *Image, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																			// GraphImageデータから分割グラフィックハンドルを作成する
extern	int			b2_CreateDivGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;													// GraphImageデータから分割グラフィックハンドルを作成する
extern	int			ReCreateDivGraphFromGraphImage( BASEIMAGE *Image, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																		// GraphImageデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			b2_ReCreateDivGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;													// GraphImageデータから既存の分割グラフィックハンドルにデータを転送する
...
extern	MATRIX		GetCameraBillboardMatrix() ;																						// ビルボード行列を取得する
...
extern	int			GetDrawValidAlphaChannelGraphCreateFlag() ;							// 描画可能なαチャンネル付き画像を作成するかどうかのフラグを取得する
extern	int			SetDrawValidFloatTypeGraphCreateFlag( int Flag ) ;					// 描画可能な浮動小数点型の画像を作成するかどうかのフラグをセットする,SetDrawValidGraphCreateFlag 関数で描画可能画像を作成するように設定されていないと効果がない、ＧＰＵが Float型の画像に対応していない場合は画像作成に失敗する( TRUE:Float型 FALSE:Int型 )
extern	int			GetDrawValidFloatTypeGraphCreateFlag() ;							// 描画可能な浮動小数点型の画像を作成するかどうかのフラグを取得する
extern	int			SetDrawValidMultiSample( int Samples, int Quality ) ;				// 描画可能な画像のマルチサンプリング設定を行う
...
extern	int			SetUseGraphBaseDataBackup( int Flag ) ;								// 画像を作成する際に使用した画像データのバックアップをしてデバイスロスト時に使用するかどうかのフラグをセットする( TRUE:バックアップをする( デフォルト )  FALSE:バックアップをしない )
extern	int			GetUseGraphBaseDataBackup() ;										// 画像を作成する際に使用した画像データのバックアップをしてデバイスロスト時に使用するかどうかのフラグを取得する
...
extern	int			SetMovieRightImageAlphaFlag( int Flag ) ;							// 動画ファイルの右半分の赤成分ををアルファ情報として扱うかどうかをセットする( TRUE:アルファ情報として扱う )
...
extern	int			GetFontCharInfo( int FontHandle, LPCTSTR Char, int *DrawX, int *DrawY, int *NextCharX, int *SizeX, int *SizeY ) ;		// 指定の文字の描画情報を取得する
...
// ベクトルの加算
VECTOR	VAdd( ref VECTOR In1, ref VECTOR In2 )
{
	return VECTOR( In1.x + In2.x, In1.y + In2.y, In1.z + In2.z ) ;
}

// ベクトルの減算
VECTOR	VSub( ref VECTOR In1, ref VECTOR In2 )
{
	return VECTOR( In1.x - In2.x, In1.y - In2.y, In1.z - In2.z ) ;
}

// ４要素ベクトルの加算
FLOAT4		F4Add( ref FLOAT4 In1, ref FLOAT4 In2 )
{
	return FLOAT4( In1.x + In2.x, In1.y + In2.y, In1.z + In2.z, In1.w + In2.w ) ;
}

// ４要素ベクトルの減算
FLOAT4		F4Sub( ref FLOAT4 In1, ref FLOAT4 In2 )
{
	return FLOAT4( In1.x - In2.x, In1.y - In2.y, In1.z - In2.z, In1.w - In2.w ) ;
}

// ベクトルの内積
float	VDot( ref VECTOR In1, ref VECTOR In2 )
{
	return In1.x * In2.x + In1.y * In2.y + In1.z * In2.z ;
}

// ベクトルの外積
VECTOR	VCross( ref VECTOR In1, ref VECTOR In2 )
{
	return VECTOR( In1.y * In2.z - In1.z * In2.y, In1.z * In2.x - In1.x * In2.z, In1.x * In2.y - In1.y * In2.x ) ;
}

// ベクトルのスケーリング
VECTOR	VScale( ref VECTOR In, float Scale )
{
	return VECTOR( In.x * Scale, In.y * Scale, In.z * Scale ) ;
}

// ４要素ベクトルのスケーリング
FLOAT4		F4Scale( ref FLOAT4 In, float Scale )
{
	return FLOAT4( In.x * Scale, In.y * Scale, In.z * Scale, In.w * Scale ) ;
}
...
// ベクトルのサイズの２乗
float	VSquareSize( ref VECTOR In )
{
	return In.x * In.x + In.y * In.y + In.z * In.z ;
}

// 行列を使った座標変換
VECTOR	VTransform( ref VECTOR InV, ref MATRIX InM )
{
	return VECTOR(
...
VECTOR	VTransformSR( ref VECTOR InV, ref MATRIX InM )
{
	return VECTOR(
...
extern	int			CheckPixelAlphaSoftImage( int SIHandle ) ;																					// ソフトウエアで扱うイメージに含まれるピクセルのアルファ値をチェックする( 戻り値   -1:エラー  0:画像にアルファ成分が無い  1:画像にアルファ成分があり、すべて最大(255)値  2:画像にアルファ成分があり、存在するアルファ値は最小(0)と最大(255)もしくは最小(0)のみ　3:画像にアルファ成分があり、最小と最大以外の中間の値がある )  
...
extern	void		DrawPixelSoftImage_Unsafe_XRGB8( int SIHandle, int x, int y, int  r, int  g, int  b ) ;										// ソフトウエアで扱うイメージの指定座標にドットを描画する(各色要素は０～２５５)、エラーチェックをしない代わりに高速ですが、範囲外の座標や ARGB8 以外のフォーマットのソフトハンドルを渡すと不正なメモリアクセスで強制終了します
extern	void		DrawPixelSoftImage_Unsafe_ARGB8( int SIHandle, int x, int y, int  r, int  g, int  b, int  a ) ;								// ソフトウエアで扱うイメージの指定座標にドットを描画する(各色要素は０～２５５)、エラーチェックをしない代わりに高速ですが、範囲外の座標や XRGB8 以外のフォーマットのソフトハンドルを渡すと不正なメモリアクセスで強制終了します
extern	int			GetPixelSoftImage(  int SIHandle, int x, int y, int *r, int *g, int *b, int *a ) ;											// ソフトウエアで扱うイメージの指定座標の色を取得する(各色要素は０～２５５)
extern	void		GetPixelSoftImage_Unsafe_XRGB8( int SIHandle, int x, int y, int *r, int *g, int *b ) ;										// ソフトウエアで扱うイメージの指定座標の色を取得する(各色要素は０～２５５)、エラーチェックをしない代わりに高速ですが、範囲外の座標や XRGB8 以外のフォーマットのソフトハンドルを渡すと不正なメモリアクセスで強制終了します
extern	void		GetPixelSoftImage_Unsafe_ARGB8( int SIHandle, int x, int y, int *r, int *g, int *b, int *a ) ;								// ソフトウエアで扱うイメージの指定座標の色を取得する(各色要素は０～２５５)、エラーチェックをしない代わりに高速ですが、範囲外の座標や ARGB8 以外のフォーマットのソフトハンドルを渡すと不正なメモリアクセスで強制終了します
...
extern	int			ReverseSoftImageH( int SIHandle ) ;																							// ソフトウエアで扱うイメージの左右を反転する
extern	int			ReverseSoftImageV( int SIHandle ) ;																							// ソフトウエアで扱うイメージの上下を反転する
extern	int			ReverseSoftImage( int SIHandle ) ;																							// ソフトウエアで扱うイメージの上下左右を反転する
...
extern	int			GetBaseImageGraphDataSize( BASEIMAGE *BaseImage ) ;																													// 基本イメージデータのイメージサイズを取得する( byte単位 )
...
extern	int			GetPanSoundMem( int SoundHandle ) ;																		// メモリに読みこんだWAVEデータの再生にパンを取得する
...
extern	int			ResetFrequencySoundMem( int SoundHandle ) ;																// メモリに読み込んだWAVEデータの再生周波数を読み込み直後の状態に戻す
...
extern	int			SaveSoftSound( int SoftSoundHandle, LPCTSTR FileName ) ;												// ソフトウエアで扱う波形データを無圧縮Wav形式で保存する
...
extern	int			MV1LoadTexture( LPCTSTR FilePath ) ;														// ３Ｄモデルに貼り付けるのに向いた画像の読み込み方式で画像を読み込む( 戻り値  -1:エラー  0以上:グラフィックハンドル )
