...
// Plugin特性
const int PPITYPE_IMPORT	 = 0x01; // プラグインはImport関数をサポートします
const int PPITYPE_EXPORT	 = 0x02; // プラグインはExport関数をサポートします
const int PPITYPE_FILEFORMAT = 0x04; // プラグインはファイルフォーマットとして機能します

// プログレスコールバックが返す戻り値
const int PPICALLBACK_ABORT	= 0;// ユーザが中止を選択した（速やかに処理を中断して下さい）
const int PPICALLBACK_OK	= 	1;// OK.（処理を続行して下さい）

// 呼び出し元へ返却する戻り値
const int PPIRET_OK			    = 0; // 正常終了
const int PPIRET_UNSUPPORTED    = 1; // このファイルはサポートしていません
const int PPIRET_NOTIMPLEMENTED = 2; // 未実装の関数です
const int PPIRET_INVALIDPARAMS  = 3; // 引数が不正です
const int PPIRET_INVALIDFILE    = 4; // ファイルが破損しています
const int PPIRET_FILEERR	    = 5; // ファイルエラーが発生しました
const int PPIRET_OUTOFMEMORY    = 6; // メモリが不足しています
const int PPIRET_ABORT		    = 7; // コールバック関数からPPICALLBACK_ABORTを受け取ったので処理を中止しました
const int PPIRET_INTERNALERR	= 8;// 内部エラーが発生しました

//const void SAFE_GLOBAL_FREE(T)(T p) if (p != NULL) { ::GlobalFree(p); p = NULL; }
