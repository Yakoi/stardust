...
//-----------------------------------------------------------------------------
/+
void GetPlatinumPluginInfo(PlatinumPluginInfo* pPluginInfo)
{
...
//-----------------------------------------------------------------------------
int IsSupported(LPCTSTR lpszFilePath)
{
	HANDLE hFile;
...
//-----------------------------------------------------------------------------
version(none){
int Export(HWND hWndParent, LPCTSTR lpszFilePath, const PlatinumData* pData, PPI_PROGRESS_CALLBACK fnCallback)
{
	...
	fmfHeader.byChipWidth	= cast(BYTE)pData.important.header.dwChipWidth;
	fmfHeader.byChipHeight	= cast(BYTE)pData.important.header.dwChipHeight;
...
//-----------------------------------------------------------------------------
/+
int Import(HWND hWndParent, LPCTSTR lpszFilePath, PlatinumData* pData, PPI_PROGRESS_CALLBACK fnCallback)
{
	if (hWndParent == NULL || lpszFilePath == NULL || pData == NULL || fnCallback == NULL)
		return PPIRET_INVALIDPARAMS;
