// D import file generated from 'material\transitiontable.d'
...
roll = tileWipeTransition(screen,new RollingTileDrawer(RotationDirection.left,1),new CircleOutTileDiffusion,16,16,0.2,0.5);
left_blind = verticalWipeTransition(screen,new DirectTileDrawer(Direction8.L),new DirectTileDiffusion(Direction8.L),10,0.2,0.5);
right_blind = verticalWipeTransition(screen,new DirectTileDrawer(Direction8.R),new DirectTileDiffusion(Direction8.R),10,0.2,0.5);
water = horizonWipeTransition(screen,new CircleTileDrawer,new CircleInTileDiffusion,20,0.2,0.5);
mist = horizonWipeTransition(screen,new UnboxTileDrawer,new CircleInTileDiffusion,10,0.2,0.5);
lines = horizonWipeTransition(screen,tdr["left"],tdr["right"],tdf["down2"],2,0.2,0.5);
test = tileWipeTransition(screen,tdr["tl"],tdf["br"],16,16,0.2,0.5);
